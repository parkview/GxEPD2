...
// See example code I used below.  I first tried sending text files to be displayed, but struggled with JSON encoding etc.  
// The idea was to include formatting commands in the uploaded text files and let the ESP8266 generate/draw info straight 
// onto the E-Paper display.
//
// The sample code souced from Jean-Marc Zingg, allows me to upload a BMP imageand display the 400x300 image.  
// This way everything is done on the Intranet server.
//
// Source URL:  http://10.0.0.30/text1.bmp
// Update cycle: 5 minutes
//
// Author: Paul Hamilton - 2019-06-16
...
#define USE_BearSSL false
#endif

#include <WiFiClient.h>
#include <WiFiClientSecure.h>

const char* ssid     = "MYHOME1";
const char* password = "miTTsoff,mystuff.";
...
void setup()
{
  ...
  InitialDisplay();  // go show the display config
}

void InitialDisplay(void)
{
   // this will show some information about the initial config od the e-Dsiplay
   ...
   u8g2Fonts.setFont(u8g2_font_helvB12_tr);  // See font list: https://github.com/olikraus/u8g2/wiki/fntlistall
   u8g2Fonts.setCursor(0, 20);
   u8g2Fonts.println();
   u8g2Fonts.print("SSID:  ");
   u8g2Fonts.println(ssid);
   u8g2Fonts.println();
   u8g2Fonts.print("Display IP:  ");
   u8g2Fonts.println(WiFi.localIP());
   u8g2Fonts.println();
   u8g2Fonts.println("URL:  http://10.0.0.30/text1.bmp");
   u8g2Fonts.println();
   u8g2Fonts.println("USB Serial speed:  115200");
   u8g2Fonts.println();
   u8g2Fonts.println("Screen refresh rate:  5 minutes");
   //while (display.firstPage());  //whats this function for?
   while (display.nextPage());
   delay(20000);  // give the user 20 seconds to view the config screen before starting the first display
}

void loop(void)
{
   int16_t Wait = 5;   // wait for 5 minutes
   int16_t x = 0;
   int16_t y = 0;
   // have to use Buffered function, otherwise only a partial BMP is displayed      
   showBitmapFrom_HTTP_Buffered("10.0.0.30", "/", "text1.bmp", x, y, false);
   delay(Wait * 60 * 1000);  // delay for over a minute to then check for a new image
}
